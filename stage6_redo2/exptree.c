...
struct Gsymbol* TypeVarlistLookup(char *varname){
	struct Ttable *temp = typeTable;
	while(1){
		struct Gsymbol *temp2 = searchPtable(varname,&temp->varlist);
		if(temp2 != NULL){
			return temp2; 
		}
		if (temp->next != NULL)
...
struct Ttable* createType(char *typename, struct PTable *varlist){
	struct Ttable *lastNode = typeTable;
	struct Ttable *temp = NULL;
	temp = (struct Ttable*)malloc(sizeof(struct Ttable));
	temp->varname = strdup(typename);
	temp->varlist = varlist;
	temp->type = var_hash(typename);
...
int evaluate(struct tnode *t,FILE *fp){
	...
	switch((t->nodetype)){
		case MEM_INIT: //xos_call(FILE *fp,char *func_code ,int arg1,int arg2)
			xos_call(fp,"Heapset",0,0);
			freeReg();


		case MEM_ALLOC: ;
				int hom_adrs = xos_call(fp,"Alloc",8,0);
				return hom_adrs;
