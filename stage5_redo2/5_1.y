...
%type <no> expr NUM TEXT ID Body END BEGIN1 Slist stmt input output asgn ifstmt whilestmt BREAK CONTINUE  
	       ...
	       GdeclBlock GdeclList GDecl LDecList LDecl FID GENDDECL GDECL TYPE_MAIN
...
%token NUM TEXT ID PLUS MINUS MUL DIV MOD EQ BEGIN1 END READ WRITE POW IF ELSE THEN ENDIF WHILE DO ENDWHILE 
	   ...
	   RET PID LIDec IdList GDeclBlock FID GDECL GENDDECL TYPE TYPE_MAIN
...
Program : 	TypeStruct GdeclBlock FdefBlock MainBlock		{printf("Program done\n");}
		  | GdeclBlock MainBlock				{printf("Program done\n");}
		  | MainBlock							{printf("Program done\n");};

TypeBlock 	: TYPE  TypeStruct ENDTYPE 			{
...
TypeStruct 	: 	TypeStruct ',' PID '{' ParamList '}' 
			|	PID '{' ParamList '}' 				 
			;
