...
struct typetable *defaultTypes(){
    ...
    temp2->name = malloc(sizeof(char));
    strcpy(temp2->name,"str");
    temp2->type = 1;			//str type def
    temp2->size = 1;
...
struct typetable *appendtype(char *name,,int size)
{
	struct typetable *temp1;
    temp1 = (struct typetable*)malloc(sizeof(struct typetable));
    temp1->name = malloc(sizeof(char));
...
struct typetable *lookupType(char *name)
{
	struct typetable *p;
	p=troot;
	while(p->next!=NULL){
